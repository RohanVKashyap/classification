...
class ClassifierModel(torch.nn.Module):
    ...
    def train_model(cls, ds : torch.utils.data.Dataset, num_labels, num_classes, label2id,
        ...
        classifier_model.fit(train_loader, loss_fn, optimizer, 100)
...
def test_prediction():
    ...
    with open('test_examples.txt','r') as f:
       for i in f:
          sentence,answer=i.split('::')
          test_examples.append(sentence)
          test_labels.append(answer.strip())

    res = [ (t, predict(t)) for t in test_examples] 

    for i,(j,k) in zip(test_labels,res):
        if str(i)==str(k):
            total_correct+=1

    accuracy=(total_correct/len(test_labels))*100
    print('accuracy:',accuracy)             
    #res = [ (t, predict(t)) for t in examples]
    #print(type(res))
    #print(res)
    #print(tabulate(res))   
